...
sebelumnya cuma bisa lewat SSO polos, sekarang dilengkapi form detail kios. Ada **dua jalur**:

1. **Dengan SSO (default/direkomendasikan)** — identitas (nama & email) datang dari Google,
   dikunci read-only di form. Ini bagian utama dokumen ini (§1–§5 di bawah).
2. **Tanpa SSO sama sekali** — email & nama diketik manual di form, tidak ada langkah Google
   sign-in di depan. Lihat §6 untuk detail dan trade-off-nya.

Untuk envelope response, error format, dan detail lengkap tiap endpoint, lihat
[`docs/api.md`](./api.md).
...
## 6. Alternatif: onboarding tanpa SSO sama sekali

Kalau frontend tidak mau memaksa Google sign-in di depan form (misal supaya user tidak perlu
punya akun Google dulu untuk sekadar mendaftar), pakai `POST /api/v1/seller/onboard-manual`
sebagai gantinya. Endpoint ini **tidak butuh JWT** — user langsung mengetik email + nama sendiri
di form yang sama, sekaligus detail kios, dalam satu request. Detail field lengkap ada di
[`docs/api.md` §`POST /api/v1/seller/onboard-manual`](./api.md).

Alur jadi jauh lebih pendek — tidak ada langkah Google sign-in / prefill sama sekali:

```
1. User isi semua field form (Nama, Email, Nomor Telepon, Nama Kios, dst.) + upload logo
2. POST /api/v1/seller/onboard-manual   multipart/form-data (tanpa Authorization header)
      → { user + seller baru dibuat sekaligus }
3. (opsional) User login nanti via Google SSO biasa — kalau emailnya sama persis dengan
   yang didaftarkan di langkah 1, backend otomatis menautkan akun tsb ke identitas Google itu,
   bukan bikin akun baru. Dari titik itu, login selanjutnya cukup lewat Google seperti biasa.
```

**Kapan pakai jalur ini vs jalur SSO (§1–§5):**

- Pakai jalur **SSO** (default) kalau ingin identitas user terverifikasi sejak awal — cocok untuk
  produk yang serius soal keaslian akun.
- Pakai jalur **manual** (§6 ini) kalau prioritasnya mengurangi friksi pendaftaran, dan risiko di
  bawah ini bisa diterima.

**Trade-off yang harus dipahami sebelum pakai jalur ini:** backend **tidak memverifikasi
kepemilikan email** saat onboarding manual — tidak ada OTP/magic link, cuma format `@` yang
dicek. Konsekuensinya, siapa pun yang tahu email seseorang bisa mendaftarkan kios di bawah email
itu duluan (dengan data mereka sendiri di field lain). Kalau pemilik asli email tsb nanti login
via Google, mereka akan "mewarisi" kios yang didaftarkan orang lain itu — bukan mendapat error.
Untuk platform ini dampaknya dianggap rendah (yang dipertaruhkan cuma profil kios, bukan
kredensial atau saldo), tapi jangan perlakukan jalur ini setara dengan verifikasi identitas SSO.

Kalau email yang dikirim sudah terdaftar (baik dari onboarding manual lain maupun dari akun Google
yang sudah pernah login), request ditolak dengan `409` — **jangan** retry ke endpoint yang sama,
arahkan user untuk sign-in via Google (§2) sebagai gantinya.
